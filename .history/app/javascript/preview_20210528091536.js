document.addEventListener('DOMContentLoaded', function () {
  if (document.getElementById('item-image-0')) { // 変化監視要素:イメージのフォーム
    ...
    const createImageHTML = (blob, e) => {
      ...
      // if (ImagesCount != 0) {
      // }
      delHTML.setAttribute('onclick', `document.getElementById('${targetId}').value = ""; this.closest(".image-element").remove()`);
      ...
      inputHTML.setAttribute('id', `item-image-${ImagesCount + 1}`);
      ...
      var d = Array.prototype.slice.call(document.querySelectorAll("[id]"), 0).filter(
        function (i) {
          return i.id.match(/^a[item-image-]\d{2}$/);
        });
      console.log(d);
