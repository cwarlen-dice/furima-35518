document.addEventListener('DOMContentLoaded', function () {
  if (document.getElementById('item-image-0')) { // 変化監視要素:イメージのフォーム
    ...
    const createImageHTML = (blob, e) => {
      ...
      // if (ImagesCount != 0) {
      // }
      delHTML.setAttribute('onclick', `document.getElementById('${NowId}').value = ""; this.closest(".image-element").remove()`);
      ...
      const ImagesCount = document.querySelectorAll('.image-element').length + 1; // カウンターセット
      ...
      console.log(NowId);
      if (NowId == 'item-image-0') {
        // ImageList.appendChild(imageElement);
        ImageList.prepend(imageElement);
      } else {
        document.getElementById(NowId).nextElementSibling(imageElement)
      }
      // console.log(e);
